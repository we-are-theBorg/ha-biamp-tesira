...
class BiampTesiraCoordinator:
    ...
    def _connect(self) -> None:
        ...
        # Compute skip list from options. Default: load all supported types.
        enabled = set(
            self.entry.options.get(
                CONF_ENABLED_BLOCK_TYPES, list(SUPPORTED_BLOCK_TYPES.keys())
            )
        )
        skip_types = [t for t in SUPPORTED_BLOCK_TYPES if t not in enabled] or None
        if skip_types:
            _LOGGER.info("Skipping %d block type(s): %s", len(skip_types), skip_types)

        dsp.connect(backend=ssh, skip_block_types=skip_types)
        self.dsp = dsp
